  ...
  try {
    ...
    if (preserveAttributes) {
      try {
        ...
        // Why cp failed decides what to say, and a bare `catch {}` here got
        // that wrong: a FULL VOLUME, a read-only mount and an unreadable
        // target all produced "its extended attributes could not be read".
        // That is the same class of misdiagnosis this file exists to remove
        // -- an honest failure reported as the wrong cause is worse than a
        // raw errno, because it sends the operator to the wrong problem.
        //
        // cp's stderr is INSPECTED here but still never forwarded: it names
        // the temp path, which sits inside a granted directory and is not a
        // path vpath.ts translates on the way out, so echoing it would trip
        // redactLeakedHostPaths and replace any explanation with a generic
        // "internal error". Reading it to classify is not the same as
        // quoting it.
        const stderrText = String(
          (seedErr as { stderr?: Buffer | string } | undefined)?.stderr ?? ''
        );
        if (/No space left on device|ENOSPC/i.test(stderrText)) {
          // Preserve the documented ENOSPC contract: the original file is
          // untouched (cp only ever wrote the temp file) and the caller is
          // told the real reason. Seeding the temp file from the target is
          // what made a full volume reachable HERE rather than at the
          // writeFileSync below, so this branch is not hypothetical.
          const nospc: NodeJS.ErrnoException = new Error(
            `ENOSPC: no space left on device, replacing ${filePath}. The file is unchanged. ` +
              `Replacing a file needs room for the old copy and the new one at the same time.`
          );
          nospc.code = 'ENOSPC';
          nospc.path = filePath;
          throw nospc;
        ...
        if (fs.existsSync(filePath) && !readable) {
        ...
        if (fs.existsSync(filePath)) {
          // Readable, present, and cp still failed: a read-only mount, a
          // missing /bin/cp, a signal. Say that, rather than asserting a
          // cause that was just ruled out.
          const other: NodeJS.ErrnoException = new Error(
            `refusing to replace ${filePath}: its extended attributes and ACL could not be copied ` +
              `onto the replacement, so replacing it (which writes a new inode) would destroy ` +
              `them. The file is unchanged.`
          );
          other.path = filePath;
          throw other;
        }
